...
function RediscoverCard({
  ...
  return (
    <Link
      ...
      className="block rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm transition hover:shadow-md overflow-hidden"
    >
      {recipe.imageUrl ? (
        <img
          src={recipe.imageUrl}
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-2">
          ...
          {recipe.difficulty && (
            <span className="shrink-0 rounded-full bg-orange-50 dark:bg-orange-950 px-2 py-0.5 text-xs font-medium text-orange-600 dark:text-orange-400">
        ...
        {recipe.tags.length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {recipe.tags.map((tag) => (
              <span
                key={tag}
                className="rounded-full bg-orange-50 dark:bg-orange-950 px-2 py-0.5 text-xs font-medium text-orange-600 dark:text-orange-400"
              >
                {tag}
              </span>
            ))}
...
export default function RediscoverPage(): React.JSX.Element {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      <h1 className="mb-6 text-2xl font-bold text-gray-900 dark:text-gray-100">Rediscover</h1>

      {/* Mode toggle */}
      <div className="mb-8 flex rounded-xl bg-gray-100 dark:bg-gray-800 p-1 w-fit">
        {MODES.map((m) => (
          <button
            key={m.value}
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-dashed border-gray-200 dark:border-gray-700 px-8 py-16 text-center">
          <p className="font-medium text-gray-700 dark:text-gray-300">{currentMode.emptyTitle}</p>
